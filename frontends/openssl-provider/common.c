...
static size_t esdm_rand_get_seed(void *ctx __unused, unsigned char **buffer,
                                 int entropy, size_t min_len __unused,
                                 ...
                                 const unsigned char *addin, size_t addin_len)
{
  struct esdm_seed_buffer {
    uint64_t len;
    uint64_t entropyBits;
    uint8_t buf[2048] /* should be large enough for entropy from all active
                         sources */
  } __attribute__((__packed__));
  size_t buffer_size = sizeof(struct esdm_seed_buffer);
  struct esdm_seed_buffer *seed_buf;
  ssize_t ret;

  *buffer = OPENSSL_secure_zalloc(sizeof(struct esdm_seed_buffer));

  if (sizeof(struct esdm_seed_buffer) < min_len)
    goto err;
  if (sizeof(struct esdm_seed_buffer) >= max_len)
    goto err;
  ...
  esdm_invoke(esdm_rpcc_get_seed(*buffer, buffer_size, 0));
  if (ret <= 0)
    goto err;

  seed_buf = (struct esdm_seed_buffer *)*buffer;
  if (seed_buf->entropyBits < (uint64_t)entropy)
    goto err;
